...
class _DataScreenState extends State<DataScreen> {
  ...
  Future<void> _pickAndUploadFile() async {
    ...
    try {
      ...
      // TODO: Add camera capture with countdown and location metadata here
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        // Update buffer pool with amount based on uploaded files
        final bufferPoolProvider = Provider.of<BufferPoolProvider>(
          context,
          listen: false,
        );
        final amountToAdd = result.files.length * 100.0; // ₹100 per file
        bufferPoolProvider.addToBufferAmount(amountToAdd);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${result.files.length} file(s) uploaded successfully. ₹${amountToAdd.toStringAsFixed(2)} added to buffer pool.',
            ),
